...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($manga_id) || empty($quantidade) || empty($data_venda)) {
        ...
        $sql = "INSERT INTO vendas (manga_id, quantidade, data_venda) VALUES (?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$manga_id, $quantidade, $data_venda]);
        header('Location: index.php?msg=Venda cadastrada com sucesso');
        exit;
    }
}
...
<div class="flex items-center space-x-4 mb-8">
    ...
    <div>
        <h1 class="text-2xl font-extrabold text-slate-800">Nova Venda</h1>
        <p class="text-slate-500 text-sm mt-1">Registre uma nova venda</p>
    </div>
</div>
    ...
    <form method="POST" class="space-y-5">
        ...
        <div class="space-y-1.5">
            ...
            <select id="manga_id" name="manga_id" required
                ...
                <?php foreach ($mangas as $manga): ?>
                    <option value="<?= $manga['id'] ?>" <?= (isset($_POST['manga_id']) && $_POST['manga_id'] == $manga['id']) ? 'selected' : '' ?>>
                        <?= htmlspecialchars($manga['titulo']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
            <?php if (empty($mangas)): ?>
                <p class="text-xs text-amber-600 mt-1">Nenhum mangá cadastrado. <a href="../LivrosCRUD/create.php" class="font-semibold underline">Cadastre um mangá primeiro.</a></p>
            <?php endif; ?>
        </div>

        <div class="space-y-1.5">
            <label for="quantidade" class="block text-sm font-semibold text-slate-700">Quantidade</label>
            <input type="number" id="quantidade" name="quantidade" required min="1" placeholder="1"
        ...
        <div class="space-y-1.5">
            ...
            <input type="date" id="data_venda" name="data_venda" required
                   value="<?= htmlspecialchars($_POST['data_venda'] ?? date('Y-m-d')) ?>"
